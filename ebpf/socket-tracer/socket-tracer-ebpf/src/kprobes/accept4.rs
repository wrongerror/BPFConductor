...
pub fn entry_accept4(ctx: TracePointContext) -> u32 {
    try_entry_accept4(ctx).unwrap_or_else(|ret| ret.try_into().unwrap_or_else(|_| 1))
}

pub const ACCEPT4_SOCKADDR_OFFSET: usize = 24;

fn try_entry_accept4(ctx: TracePointContext) -> Result<u32, i64> {
    // let sockaddr: *const sockaddr = ctx.arg(1).ok_or(1i64)?;
    let sockaddr = unsafe { ctx.read_at::<*const sockaddr>(ACCEPT4_SOCKADDR_OFFSET)? };
...
pub const ACCEPT4_RET_OFFSET: usize = 16;
#[tracepoint]
pub fn ret_accept4(ctx: TracePointContext) -> u32 {
    try_ret_accept4(ctx).unwrap_or_else(|ret| ret.try_into().unwrap_or_else(|_| 1))
}

fn try_ret_accept4(ctx: TracePointContext) -> Result<u32, i64> {
    let pid_tgid = bpf_get_current_pid_tgid();
    let accept_args = unsafe { ACTIVE_ACCEPT_MAP.get(&pid_tgid).ok_or(1i64)? };
    let res = process_syscall_accept4(&ctx, pid_tgid, accept_args);
...
fn process_syscall_accept4(
    ctx: &TracePointContext,
    pid_tgid: u64,
    args: &types::AcceptArgs,
) -> Result<u32, i64> {
    let tgid: u32 = (pid_tgid >> 32) as u32;
    let ret_fd: i32 = unsafe { ctx.read_at(ACCEPT4_RET_OFFSET)? };
