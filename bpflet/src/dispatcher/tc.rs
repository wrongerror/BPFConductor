...
impl TcDispatcher {
    pub(crate) async fn new(
    ...
    ) -> Result<TcDispatcher, BpfletError> {
        ...
        image_manager
            ...
            .await
            .map_err(|e| BpfletError::RpcSendError(e.into()))?;
        let program_bytes = rx.await?.map_err(BpfletError::BpfBytecodeError)?;
    ...
    async fn attach(&mut self, old_dispatcher: Option<Dispatcher>) -> Result<(), BpfletError> {
        debug!(
        ...
        if TcDispatcher::has_qdisc("clsact".to_string(), self.if_index as i32).await? {
            ...
            return Err(BpfletError::InvalidAttach(format!(
        ...
        for (i, v) in extensions.iter_mut().enumerate() {
            if v.get_attached()? {
                ...
                let mut loader = bpf
                    ...
                    .map_err(BpfletError::BpfLoadError)?;

                let ext: &mut Extension = loader
                    .program_mut(name)
                    .ok_or_else(|| BpfletError::BpfFunctionNameNotValid(name.to_string()))?
                    .try_into()?;
    ...
    fn save(&self) -> Result<(), BpfletError> {
        debug!(
        ...
        serde_json::to_writer(&fs::File::create(path).unwrap(), &self)
            .map_err(|e| BpfletError::Error(format!("can't save state: {e}")))?;
        Ok(())
    }
