...
impl TryFrom<AyaProgInfo> for KernelProgramInfo {
    ...
    fn try_from(prog: AyaProgInfo) -> Result<Self, Self::Error> {
        Ok(KernelProgramInfo {
            ...
            map_ids: prog.map_ids().map_err(BpfletError::BpfProgramError)?,
            ...
            bytes_memlock: prog.memory_locked().map_err(BpfletError::BpfProgramError)?,
...
impl TryFrom<&Program> for V1KernelProgramInfo {
    ...
    fn try_from(program: &Program) -> Result<Self, Self::Error> {
        // Get the Kernel Info.
        let kernel_info = program.kernel_info().ok_or(BpfletError::Error(
...
impl Program {
    ...
    pub(crate) fn data_mut(&mut self) -> Result<&mut ProgramData, BpfletError> {
        match self {
    ...
    pub(crate) fn data(&self) -> Result<&ProgramData, BpfletError> {
        match self {
