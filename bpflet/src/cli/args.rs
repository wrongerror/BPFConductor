...
pub(crate) enum Commands {
    /// Load an eBPF program from a local .o file or from a remote OCI container image.
    /// Example: bpflet load --path /run/bpflet/examples/go-xdp-counter/bpf_bpfel.o --name xdp_pass --iface eth0 --priority 1
    #[command(subcommand)]
    Load(LoadSubcommand),
...
pub(crate) struct LoadFileArgs {
    ...
    /// Example: --path /run/bpflet/examples/go-xdp-counter/bpf_bpfel.o
